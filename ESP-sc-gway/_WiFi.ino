...
#	if _MONITOR>=1
  if (debug >= 1) {
    ...
    mPrint("Connect Wifi to accesspoint: " + String(AP_NAME) + " and connect to IP: 192.168.101.101");
  }
#	endif //_MONITOR
...
int WlanConnect(int maxTry) {
  ...
  unsigned long timestamp_conexao_wifi;
  const unsigned long tempo_max_sem_conexao_wifi = 300000; //ms
  ...
  // Init wi-fi connection temporization
  timestamp_conexao_wifi = millis();
...
#				if _MONITOR>=1
        if ( debug >= 0 ) {
          ...
          // Tolerance to successfully to a wifi router
          if ( (millis() - timestamp_conexao_wifi) >= tempo_max_sem_conexao_wifi)
          {
            Serial.println("ERROR: waited too long to connect to the wi-fi router. ESP32 will reboot within 1 sec ...");
            delay(1000);
            ESP.restart();
          }
        }
